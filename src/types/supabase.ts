...
export interface ProctorXTableType {
  ...
  email: string; // User's login email (must be unique and not null)
  pass: string;  // Plaintext password (SECURITY RISK - FOR TESTING ONLY, not null)
  name: string;  // User's full name (not null)
  role: 'student' | 'teacher'; // User's role (not null)
  avatar_url: string | null; // URL for the user's avatar
  created_at?: string; // Auto-generated by DB
  // updated_at?: string; // Can be managed by DB trigger
}

export type ExamStatus = 'Published' | 'Ongoing' | 'Completed'; // 'Draft' removed
...
export type FlaggedEventType =
  ...
  | 'copy_attempt'
  | 'paste_attempt'
  | 'shortcut_attempt' // Added for SEB utils
  | 'dev_tools_detected' // Added for SEB utils
  | 'webdriver_detected'; // Added for SEB utils


export interface FlaggedEvent {
  type: FlaggedEventType;
  timestamp: Date; // Will be stored as ISO string in JSONB
  studentId: string; // student_user_id from proctorX
  examId: string; // exam_id from ExamX
  details?: string;
}
  ...
  public: {
    Tables: {
      proctorX: {
        ...
        Update: Partial<Omit<ProctorXTableType, 'created_at' | 'user_id' | 'email' | 'role'>>; // email and role typically not changed here
      };
      ExamX: {
        Row: {
          exam_id: string; // UUID
          ...
          status: ExamStatus; // 'Published', 'Ongoing', 'Completed'
      ...
      ExamSubmissionsX: {
        Row: {
          ...
          flagged_events: FlaggedEvent[] | null; // Array of FlaggedEvent objects
        };
        Insert: {
