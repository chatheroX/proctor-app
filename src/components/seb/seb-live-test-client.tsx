...
export function SebLiveTestClient() {
  ...
  useEffect(() => {
    ...
    console.log("[SebLiveTestClient] SEB environment confirmed.");
  ...
  const fetchExamData = useCallback(async () => {
    ...
    try {
      ...
      const { error: submissionUpsertError } = await supabase
        ...
        }, { onConflict: 'exam_id, student_user_id', ignoreDuplicates: false })
        .select();
  ...
  }, [examIdFromUrl, supabase, studentUser, isValidSession, toast, setPageError, setPageIsLoading, setExamDetails, setQuestions]);
  ...
  useEffect(() => {
    if (!isClientSide || !isSebEnvironment()) return;
    ...
    const beforeUnloadHandler = (event: BeforeUnloadEvent) => {
        console.warn("[SebLiveTestClient] Attempt to unload/refresh page blocked (simulated).");
  ...
  }, [studentUser?.user_id, examDetails, supabase, toast]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 to-slate-950 p-4 text-center">
        ...
        <h2 className="text-xl font-medium text-slate-200 mb-1">
          {authLoading ? "Authenticating User..." :
           isValidSession === undefined ? "Validating SEB Session..." : 
           "Loading Live Exam..."}
        </h2>
        ...
        <Card className="w-full max-w-md modern-card text-center shadow-xl bg-card/80 backdrop-blur-lg border-border/30">
          ...
          <CardContent className="pb-6">
            <p className="text-sm text-muted-foreground mb-6">{pageError || "Invalid exam session. SEB will quit."}</p>
             <Button onClick={() => { if(typeof window !== 'undefined') window.location.href="seb://quit"}} className="w-full btn-gradient-destructive">Exit SEB</Button>
          </CardContent>
        </Card>
      </div>
    );
