...
export function SebExamViewClient() {
  ...
  useEffect(() => {
    ...
    console.log("[SebExamViewClient] Checking SEB environment...");
    if (!isSebEnvironment()) {
    ...
    console.log("[SebExamViewClient] SEB environment detected.");
    ...
    if (!examIdParam || !tokenParam) {
      ...
      toast({ title: "Launch Error", description: "Invalid SEB parameters.", variant: "destructive" });
      setIsLoading(false);
      return;
    }
    setExamIdFromToken(examIdParam);
    setTokenFromHash(tokenParam);
    // Next effect handles decryption and data fetching
  }, [isClientSide, router, toast]);


  // Step 2: Decrypt token and fetch exam details (once studentUser and token are available)
  useEffect(() => {
    if (!isClientSide || !examIdFromToken || !tokenFromHash || authLoading) {
        if(isClientSide && !authLoading && (!examIdFromToken || !tokenFromHash) && isLoading && !error) {
            // This means params were not set from hash correctly, and we are not already in an error state
            // from the hash parsing effect.
             setError("SEB launch parameters missing or invalid. Cannot proceed.");
             setIsLoading(false);
        }
        return;
    }
    ...
    decryptData<DecryptedTokenPayload>(tokenFromHash)
      .then(payload => {
        ...
        // Fetch Exam Details
        setIsLoading(true); // Set loading for this fetch
        supabase.from('ExamX')
          .select('*')
  ...
  const performSystemChecksAndStartExam = useCallback(async () => {
    ...
    console.log("[SebExamViewClient] System Check Results:", checkResults.join('\\n'));
    toast({
