        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center gap-3">
            <Avatar className="h-9 w-9">
              <AvatarImage src={isDemoMode ? `https://api.dicebear.com/8.x/micah/svg?seed=teacher-demo` : studentRollNumber ? `https://api.dicebear.com/8.x/micah/svg?seed=${studentRollNumber}` : undefined} alt={studentName || 'User'} />
              <AvatarFallback>{studentName ? studentName.charAt(0).toUpperCase() : 'U'}</AvatarFallback>
            </Avatar>
            <div>
              <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{studentName || (isDemoMode ? "Teacher Demo" : "Student")}</p>
